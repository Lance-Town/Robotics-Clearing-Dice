...
robot = robot(BunsenIP)
robot.set_speed(300)
SCHUNK_SLEEP_TIME = 2.0
bunsen_home = [0, 0, 0, 0,-90, 30]
bunsen_dice_pos_ready_catesian = [444, -30, 83, -178, 0,0]
bunsen_dice_pos_start_cartesian = [640, -173, -200, -178, 0, 0] 
diceIteration = 0
...
This is some MQTT Code that I was messing with this weekend. Ignore it
...
def on_message(client, userdata, msg):
    ...
    if (type == "TakePicture"):
        ...
        response = genJSON("PickingUpDice", bunsenCoords[0][diceIterator])
        diceIterator += 1
        client.publish()


# IGNORE -- Testing this function, it currently doesn't fully work
...
def takePicture() -> list:
    time.sleep(2)
    tmpCoords = sorting.bunsenGetCoords()
    time.sleep(2)
    return tmpCoords
...
def goToHome():
    openGripper()
    robot.write_joint_pose(bunsen_home)

# Place dice neatly on table
def placeDice():
    robot.write_cartesian_position(bunsen_dice_pos_ready_catesian)
    robot.write_cartesian_position(bunsen_dice_pos_start_cartesian)
    # print(bunsen_dice_pos_start_cartesian)
    openGripper()
    bunsen_dice_pos_start_cartesian[2] += 70
    robot.write_cartesian_position(bunsen_dice_pos_start_cartesian)
    bunsen_dice_pos_start_cartesian[2] -= 70
    ...
    if ((diceIteration % 3) == 0):
        bunsen_dice_pos_start_cartesian[0] -= 160 
        bunsen_dice_pos_start_cartesian[1] -= (160*2)
    else:
        bunsen_dice_pos_start_cartesian[1] += 160
    # print(bunsen_dice_pos_start_cartesian)
    robot.write_cartesian_position(bunsen_dice_pos_ready_catesian)

# Grab a dice from a passed in coordinate
...
# opens the gripper and adds a sleep timer
def openGripper():
    robot.onRobot_gripper(width_in_mm=100, force_in_newtons=80, wait=False)
    time.sleep(1.0)
    # time.sleep(SCHUNK_SLEEP_TIME)

# closes the gripper and adds a sleep timer
def closeGripper():
    robot.onRobot_gripper(width_in_mm=50, force_in_newtons=40, wait=False)
    time.sleep(1.0)

def main():
    goToHome()

    # Bunsen gets a sorted list of coordinates to go to
    bunsenCoords = sorting.getBunsenCoords()

    for coord in bunsenCoords:
